...
class LangXq : Lang() {
    ...
    init {
        ...
        // Matching @Attributes
        _fallthroughStylePatterns.add(Arrays.asList(Prettify.PR_LITERAL, Pattern.compile("^\\@[\\w-]+")))
        // Matching xml tags
        _fallthroughStylePatterns.add(
            Arrays.asList(
                Prettify.PR_TAG, Pattern.compile("^<\\/?[a-z](?:[\\w.:-]*\\w)?|\\/?>$", Pattern.CASE_INSENSITIVE)
            )
        )
        // Matching single or multiline xquery comments -> (: <text> :)
        _fallthroughStylePatterns.add(
            Arrays.asList(
                Prettify.PR_COMMENT, Pattern.compile("^\\(:[\\s\\S]*?:\\)")
            )
        )
        // Tokenizing /{}:=;*,[]() as plain
        _fallthroughStylePatterns.add(
        ...
        _fallthroughStylePatterns.add(
            Arrays.asList(
                Prettify.PR_KEYWORD, Pattern.compile("^(?:xquery|where|version|variable|union|typeswitch|treat|to|then|text|stable|sortby|some|self|schema|satisfies|returns|return|ref|processing-instruction|preceding-sibling|preceding|precedes|parent|only|of|node|namespace|module|let|item|intersect|instance|in|import|if|function|for|follows|following-sibling|following|external|except|every|else|element|descending|descendant-or-self|descendant|define|default|declare|comment|child|cast|case|before|attribute|assert|ascending|as|ancestor-or-self|ancestor|after|eq|order|by|or|and|schema-element|document-node|node|at)\\b")
            )
        )
        ...
        _fallthroughStylePatterns.add(
            Arrays.asList(
                Prettify.PR_TYPE,
                Pattern.compile("^(?:xs:yearMonthDuration|xs:unsignedLong|xs:time|xs:string|xs:short|xs:QName|xs:Name|xs:long|xs:integer|xs:int|xs:gYearMonth|xs:gYear|xs:gMonthDay|xs:gDay|xs:float|xs:duration|xs:double|xs:decimal|xs:dayTimeDuration|xs:dateTime|xs:date|xs:byte|xs:boolean|xs:anyURI|xf:yearMonthDuration)\\b"),
                null
            )
        )
        ...
        _fallthroughStylePatterns.add(
            Arrays.asList(
                Prettify.PR_PLAIN, Pattern.compile("^[A-Za-z0-9_\\-\\:]+")
            )
        )
