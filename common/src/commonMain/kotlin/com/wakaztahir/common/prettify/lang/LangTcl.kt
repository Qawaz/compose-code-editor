...
class LangTcl : Lang() {
    companion object {
        val fileExtensions: List<String>
            get() = Arrays.asList("tcl")
    }

    init {
        val _shortcutStylePatterns: MutableList<List<Any?>> = ArrayList()
        val _fallthroughStylePatterns: MutableList<List<Any?>> = ArrayList()
        _shortcutStylePatterns.add(Arrays.asList("opn", Pattern.compile("^\\{+"), null, "{"))
        _shortcutStylePatterns.add(Arrays.asList("clo", Pattern.compile("^\\}+"), null, "}"))
        ...
        _fallthroughStylePatterns.add(
            Arrays.asList(
                Prettify.PR_LITERAL, Pattern.compile("^\\'(?:-*(?:\\w|\\\\[\\x21-\\x7e])(?:[\\w-]*|\\\\[\\x21-\\x7e])[=!?]?)?")
            )
        )
        // A word that optionally ends with = ! or ?.
        _fallthroughStylePatterns.add(
            Arrays.asList(
                Prettify.PR_PLAIN, Pattern.compile("^-*(?:[a-z_]|\\\\[\\x21-\\x7e])(?:[\\w-]*|\\\\[\\x21-\\x7e])[=!?]?")
            )
        )
